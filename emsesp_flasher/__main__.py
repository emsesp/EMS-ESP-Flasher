...
from emsesp_flasher import const

import re

from emsesp_flasher.common import (
    ESP32ChipInfo,
    Esp_flasherError,
    chip_run_stub,
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        prog=f"emsesp_flasher {const.__version__}")
    parser.add_argument(
        "-p", "--port", help="Select the USB/COM port for uploading.")
    ...
    parser.add_argument(
        "--show-logs", help="Only show logs", action="store_true")
...
def show_logs(serial_port):
    print("Showing EMS-ESP Console:")
    print()

    # 7-bit ANSI sequences
    ...
    with serial_port:
        while True:
            ...
            # remove prompts
            if "ems-esp:# " in line:
                line = line.replace("ems-esp:# ", "")

            # remove show command - dirty hack and don't show console logins or banner
            if (line == "sshshoshowshow show lshow loshow log") or ("[shell]" in line) or ('│' in line) or ('─' in line):
                hide_line = True

            if not hide_line and line != "":
                try:
                    print(line)
...
def run_emsesp_flasher(argv):
    ...
    mock_args = configure_write_flash_args(
        ...
        firmware,
        flash_size, args.bootloader, args.partitions, args.otadata,
    ...
    try:
        ...
        raise Esp_flasherError(
            f"Error setting flash parameters: {err}") from err
    ...
    if not args.no_erase:
        try:
            ...
            raise Esp_flasherError(
                f"Error while erasing flash: {err}") from err
...
def main():
    try:
        if len(sys.argv) <= 1:
            from emsesp_flasher import gui

            return gui.main() or 0
        return run_emsesp_flasher(sys.argv) or 0
