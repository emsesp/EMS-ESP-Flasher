...
import emsesp_flasher.own_esptool as esptool

from emsesp_flasher.const import (
    ESP32_DEFAULT_PARTITIONS,
    HTTP_REGEX,
    __version__,
)
from emsesp_flasher.helpers import prevent_print
...
def read_chip_info(chip):
    ...
    if isinstance(chip, esptool.ESP32ROM):
        ...
        frequency = next((x for x in ("160MHz", "240MHz")
                         if x in features), "80MHz")
...
def read_firmware_info(firmware):
    ...
    if magic == esptool.ESPLoader.ESP_IMAGE_MAGIC:
        flash_freq_raw = flash_size_freq & 0x0F
        flash_mode = {0: "qio", 1: "qout",
                      2: "dio", 3: "dout"}.get(flash_mode_raw)
        flash_freq = {0: "40m", 1: "26m", 2: "20m",
                      0xF: "80m"}.get(flash_freq_raw)
        flag_factory = False
...
def open_downloadable_binary(path):
    ...
    try:
        ...
        raise Esp_flasherError(
            f"Error opening binary '{path}': {err}") from err
...
def format_partitions_path(path, model, flash_size):
    return path.replace("$MODEL$", model).replace("$FLASH_SIZE$", flash_size)


def configure_write_flash_args(
    info, chip, flag_factory,
    firmware_path,
    flash_size, bootloader_path, partitions_path, otadata_path,
    input, secure_pad, secure_pad_v2, min_rev, min_rev_full, max_rev_full, elf_sha256_offset,
    use_segments, flash_mmu_page_size, pad_to_size, spi_connection, output
):
    addr_filename = []
    firmware = open_downloadable_binary(firmware_path)
    flash_mode, flash_freq, flag_factory, firmware_size = read_firmware_info(
        firmware)
    ...
    if (isinstance(info, ESP32ChipInfo)):
        ...
        if (isinstance(info, ESP32ChipInfo)) and not flag_factory:   # esp32 and no factory image
            ...
            emsesp_flasher_ver = "EMS-ESP-Flasher_" + __version__

            # figure out which bootloader to use
            bootloaderstring = "bootloader_" + flash_mode + "_" + flash_freq + ".elf"
            boot_loader_path = join(
                uwd, emsesp_flasher_ver, "bootloader", model, "bin")
            ...
            try:
                # check for local elf bootloader file
                open(boot_loader_file, "rb")
            except IOError as err:              # if not found download elf bootloader file from GH repo
                boot_elf_path = open_downloadable_binary(
                    format_bootloader_path(
                        bootloader_path, model, flash_mode, flash_freq)
                )
            ...
            # next is the partitions file
            # EMS-ESP has two variants, for 4MB and 16MB flash
            if not partitions_path:
                partitions_path = format_partitions_path(
                    ESP32_DEFAULT_PARTITIONS, model, flash_size)
        ...
        # ESP8266
        raise Esp_flasherError(f"Only ESP32 supported")
