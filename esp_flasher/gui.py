...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             QHBoxLayout, QPushButton, QLabel, QComboBox, QCheckBox,
...
class FlashingThread(threading.Thread):
    def __init__(self, firmware, port, no_erase, show_logs=False):
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        firmware_layout.addWidget(firmware_label, 1, 0)
        firmware_layout.addWidget(self.firmware_button, 1, 1)
    ...
    def select_no_erase(self, state):
        self._no_erase = self.firmware_no_erase_checkbox.isChecked()
    ...
    def view_logs(self):
        self.console.clear()
        if self._port:
            worker = FlashingThread('dummy', self._port, self._no_erase, show_logs=True)
            worker.start()
